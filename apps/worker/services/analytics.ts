...
export interface ShiftMetrics {
  totalShifts: number
  totalVolunteersScheduled: number
  weeklyHoursCovered: number
  coverageSlots: ShiftCoverageSlot[]
}
...
export class AnalyticsService {
  ...
  async getConversationMetrics(
    ...
    // Average first-response time via raw SQL CTE (requires joining messages to conversations).
    const responseTimeResult = await this.db.execute(sql`
      ...
      SELECT COALESCE(ROUND(AVG(gap_seconds)), NULL)::text AS avg_seconds
