...
async function checkObjectStorage(env: Record<string, unknown>): Promise<CheckResult> {
  const endpoint = env.S3_ENDPOINT as string | undefined
  if (!endpoint) return { status: 'failing', detail: 'S3_ENDPOINT not configured' }
  const t0 = Date.now()
  try {
...
async function runChecks(env: Record<string, unknown>): Promise<HealthResult> {
  const [postgres, objectStorage, relay, sipBridge] = await Promise.all([
    checkPostgres(),
  ...
  const checks: Record<string, CheckResult> = { postgres, objectStorage, relay }
