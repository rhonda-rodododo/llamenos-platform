...
- Server WebSocket secret (can impersonate server on relay)
...
The server's WebSocket signing key and event encryption key are derived independently from `SERVER_NOSTR_SECRET` using separate HKDF labels. A signing key compromise (attacker can publish fake events) does NOT compromise event content confidentiality, and vice versa. When assessing server compromise scope, determine which derived key was exposed.
