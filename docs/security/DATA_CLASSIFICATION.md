...
#### WebSocket Relay (WebSocket relay, self-hosted)
...
|------|---------|---------|
| 2026-05-03 | 2.1 | Post-hardening: added `ua` (SHA-256 hashed) field to audit logs; noted country is not collected; updated WebSocket relay events (epoch-rotating per-hub key, power-of-2 padding, write-policy publisher constraint) |
| 2026-05-02 | 2.0 | Complete rewrite: HPKE replaces ECIES for all key wrapping, per-device Ed25519/X25519 keys replace nsec, added sigchain/PUK/CLKR entries, added CMS data, added hub key distribution, added blind indexes, updated client storage to Tauri Store/Keychain/Keystore (not localStorage), updated WebSocket relay data, added signal-notifier sidecar, removed Durable Objects/Cloudflare references |
