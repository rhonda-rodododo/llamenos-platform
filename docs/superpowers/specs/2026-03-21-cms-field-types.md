...
Verify whether `ClientReportFieldType` is a handwritten Swift enum or a generated codegen type before implementing. If codegen-generated (present in `packages/protocol/generated/swift/`), a schema registry update is required first — run `bun run codegen` after registering. If handwritten, add the `location` case directly. Route `ClientReportFieldType.location` to `LocationField` in `fieldInput(for:)` and remove the fallthrough to the default case.
...
Verify whether `LocationResult` is registered in `schema-registry.ts` before using it in Android. Run `grep -r 'LocationResult' packages/protocol/` to confirm. If absent, add it to the registry and run `bun run codegen` first — do not hand-write the class.
...
> **V1 scope:** V1 required properties: name, label, type (select picker), required toggle, and type-specific basic options (options list for select/multiselect, maxLength for text/textarea). Deferred to V2: `showWhen` conditional visibility, `access` RBAC overrides, display options (`section`, `helpText`, `placeholder`). The editor must be functional and usable without these deferred features.

Required property editors per field:
- **Basic (V1):** `label` (text), `name` (slug-format, auto-derived from label on create), `type` (select: text/textarea/number/select/multiselect/checkbox/date/file/location), `required` (toggle), `order` (drag handle or numeric).
- **Type-specific (V1):**
  ...
  - `text` / `textarea`: `validation.maxLength`.
  - `number`: `validation.min`, `validation.max`.
- **Display (V2):** `section` (text, grouping label), `helpText` (text), `placeholder` (text).
- **Access (V2):** `accessLevel` (select: all/admin/assigned/custom), `visibleToUsers` (toggle), `editableByUsers` (toggle).
- **Conditional visibility (V2):** `showWhen.field` (select from other field names in the type), `showWhen.operator` (equals/not_equals/contains/is_set), `showWhen.value`.

The V1 editor uses a simple flat layout. V2 may introduce tabbed layout: **Basic | Options | Display | Access | Conditions**.
...
| `packages/protocol/tools/schema-registry.ts` | Register `locationFieldValueSchema` as `LocationFieldValue` for mobile codegen |
...
### Codegen
After adding `locationFieldValueSchema` and `fileFieldValueSchema` to protocol schemas, run `bun run codegen` to regenerate Swift and Kotlin types.

Register `locationFieldValueSchema` in `schema-registry.ts` as `LocationFieldValue`. Mobile platforms decode the location value client-side after E2EE decryption — having a named type in generated Swift/Kotlin prevents manual JSON parsing.
...
> **V1 limitation:** Server-side precision enforcement is out of scope for V1 — precision is capped client-side before submission. Track as a future improvement: add a validation middleware that strips excess coordinates from location field values.
