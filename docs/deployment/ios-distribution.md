...
Defect 1 is fixed; the rest are recorded and depend on other work.
...
1. Confirm/create the App Store Connect app record for `org.llamenos.hotline`, and add internal testers.
2. Defect 2 (`destination`) — handled by PR #977, not this doc.
3. Populate the signing secrets (#745), or drop the `APPLE_API_KEY_*` trio if upload is only ever local.
4. Run the export on the Mac with the profile installed to exercise the defect-1 fix, then upload with `fastlane beta`.
