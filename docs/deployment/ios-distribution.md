...
**Status: decision document.** It states what this repo is configured to do today,
what that means for a person holding a downloaded iOS build, and the options for
getting internal beta testers a build they can install. It does **not** make the
decision — see [Decision needed](#decision-needed-from-the-operator).
...
Also note that **nothing installable is currently published on GitHub for iOS**:

- **Verified (repo):** `mobile-release.yml` publishes `ios-v<version>` containing
  `ios-build-meta.json` **only** (`{version, buildNumber}`). The signed IPA, if the
  job ever produces one, is a 30-day *workflow artifact* (`ios-release`), not a
  release asset. (Immutable releases forbid attaching it after the release is
  published, which is why the metadata-only release exists.)
- **Verified (repo, `gh release list`):** as of v0.19.16 the releases are `v0.19.16`,
  `android-v0.19.16` and `desktop-v0.19.16`. There is **no `ios-v*` release at all**.
  `android-v0.19.16` carries `app-release.apk`, `app-release.apk.sha256`,
  `android-build-meta.json` — Android is installable from a download; iOS is not.
...
| Export destination | `upload` | `apps/ios/ExportOptions.plist` |
| Signing style at export | `manual` | `apps/ios/ExportOptions.plist` |
| Profile name the export asks for | `match AppStore org.llamenos.hotline` | `ExportOptions.plist`, `fastlane/Fastfile` |
...
These are recorded rather than fixed, because fixing them depends on the decision
below.

1. **Profile name mismatch (Verified, Mac).** The profile *file* is named
   `match AppStore org.llamenos.hotline.mobileprovision`, but its **internal `Name`** is
   `org.llamenos.hotline AppStore`. `xcodebuild -exportArchive` resolves
   `provisioningProfiles` by the profile's internal `Name`, not its filename, so
   `ExportOptions.plist` and `fastlane/Fastfile` (both say
   `match AppStore org.llamenos.hotline`) will not find it and the export fails
   (#744/#745). Either the profile must be regenerated (e.g. via `fastlane match`,
   which names profiles `match AppStore <id>`) or the plist/Fastfile must reference
   the name that exists. This PR changes neither.
2. **`destination = upload` conflicts with the "CI never submits" policy
   (Verified repo; behaviour Apple-documented, not exercised).** Per #969, CI builds
   and publishes artifacts and never uploads to a store. With `method =
   app-store-connect`, `destination = upload` makes `xcodebuild -exportArchive`
   upload straight to App Store Connect and, as far as Apple documents it, does not
   leave an `.ipa` in the export path — while `mobile-release.yml` then tries to
   archive `build/export/*.ipa`. `destination = export` is the setting that
   produces a local IPA. This has **not** been run (it needs the secrets), so treat it
   as a strong suspicion to confirm, not a confirmed failure. It belongs in the same
   change as the method decision.
3. **Archive step has no explicit profile/identity (Unverified).** The archive step
...
4. **`APPLE_API_KEY_*` secrets are not used by the build.** (Verified, repo.)
...
- The IPA exported with `method = app-store-connect` (what the repo already has),
  fixed per defects 1–2 above, and uploaded to App Store Connect. Per #969 that
...
- `ExportOptions.plist` with `method = ad-hoc` (and `destination = export`).
  **Not changed in this PR.** Switching changes which secrets and profile are needed.
...
| Works today | No — needs secrets, app record, fix defects 1–2 | No — needs new profile + secrets | Partially — identity exists; Mac-mini host issue unresolved |
...
**Use TestFlight internal testing for the internal beta, and keep Local Xcode as the
...
1. **TestFlight vs Ad Hoc** (this sets `method` in `apps/ios/ExportOptions.plist`,
   `destination`, and which secrets/profile are required). Deliberately not changed here.
2. If TestFlight: confirm/create the App Store Connect app record, then fix defect 1
   (profile name) and defect 2 (`destination`), then populate the seven secrets
   (#745) — or drop the `APPLE_API_KEY_*` trio and the verifier if TestFlight is
   only ever uploaded locally.
3. If Ad Hoc: create the Ad Hoc profile, choose an HTTPS host for `manifest.plist`
   and the IPA, and update the workflow to publish them somewhere installable
   (the `ios-v<version>` release is metadata-only today).

Until one of these is done, **the honest answer to "can users install a
downloaded iOS app?" is no** — and the docs, release notes and tester guide should
not imply otherwise.
